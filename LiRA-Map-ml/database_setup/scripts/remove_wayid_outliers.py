...
def get_friction_data(trip_id: str,
    ...
    friction_data['averaged_way_id'] = friction_data['Way_id'].rolling(
        window = 7, 
        center=True, 
        min_periods=1).apply(lambda x: x.mode()[0]).astype(int).astype(str)
...
def remove_wayid_outliers() -> None:
    '''
        script to be rn after a trip has been fully uploaded to friction table
        --> smoothens the ways of one trip out, keeping always the most common way of 7 ways.
    '''
    # set the arguments
